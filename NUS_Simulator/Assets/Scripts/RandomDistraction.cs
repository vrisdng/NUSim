...
public class RandomDistraction : MonoBehaviour
{
    ...
    void InitializeDistractions()
    {
        ...
        distractions[4] = new DistractionEvent("You suddenly remember that hilarious meme you saw last week and must find it to show your roommate.", 0.8f, new Points(6, 0, 0));
        ...
        distractions[8] = new DistractionEvent("Netflix just released a new episode of your favorite series. One episode can't hurt, can it?", 0.9f, new Points(7, 0, 0));
        distractions[9] = new DistractionEvent("Your room is suddenly too messy to concentrate. Time for a cleaning spree!", 0.8f, new Points(5, 2, 1));
        distractions[10] = new DistractionEvent("You get a notification that there's a sale on your favorite online store. Shopping cart, here I come!", 0.7f, new Points(6, 0, 0));
        distractions[11] = new DistractionEvent("You start daydreaming about your next vacation. Mental vacation planning is productive, right?", 0.6f, new Points(3, 0, 1));
        distractions[12] = new DistractionEvent("You decide to reorganize your entire computer file system. This is definitely productive procrastination.", 0.8f, new Points(5, 0, 1));
        distractions[13] = new DistractionEvent("Your roommate starts playing your favorite song loudly. Dance party break?", 0.7f, new Points(4, 0, 2));
        ...
        distractions[15] = new DistractionEvent("A random philosophical question pops into your head. Time to deep-dive into an existential crisis.", 0.8f, new Points(6, 0, 0));
        distractions[16] = new DistractionEvent("Your favorite YouTuber just uploaded a new video. Immediate viewing required.", 0.9f, new Points(7, 0, 0));
        distractions[17] = new DistractionEvent("You realize you haven't checked social media in the last 10 minutes. Social obligations must be met.", 0.7f, new Points(5, 0, 0));
        distractions[18] = new DistractionEvent("You hear an ice cream truck outside. Ice cream is essential for studying, obviously.", 0.5f, new Points(3, 0, 1));
